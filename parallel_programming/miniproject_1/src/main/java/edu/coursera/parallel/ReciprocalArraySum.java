...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            if (endIndexExclusive - startIndexInclusive <= (input.length / ForkJoinPool.getCommonPoolParallelism())) {
                computeDirectly();
                return;
            }
    ...
    protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
        if (input == null || input.length == 0 || numTasks <= 0) {
            return 0;
        }
        ReciprocalArraySumTask task = new ReciprocalArraySumTask(0, input.length, input);
        // This needs to be done, as the default parallelism is cores - 1, but in
        // tests they are checking times for 4 cores
